...
SELECT sub_id AS post_id, COUNT(DISTINCT(com_id)) AS number_of_comments  FROM (
	SELECT t1.sub_id,t2.sub_id AS com_id FROM 
	(SELECT * FROM Submissions WHERE parent_id IS NULL) AS t1 
	LEFT JOIN (SELECT * FROM Submissions WHERE parent_id IS NOT NULL) AS t2 ON t1.sub_id=t2.parent_id
) X 
GROUP BY sub_id